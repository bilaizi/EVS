...
public class Server7 {
    ...
    static {
        try {
            ...
            privateKey = RSA.getPrivateKey("privatekey7.dat");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6007, 100, InetAddress.getByName("192.168.0.137"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5007) {
                    ...
                    if (flag) {
                        ...
                        Sender sender7 = new Sender();
                        sender7.setHost("192.168.0.137");
                        ds1.setSender(sender7);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender7.getHost()));

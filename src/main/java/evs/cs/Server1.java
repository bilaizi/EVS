...
public class Server1 {
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6001, 100, InetAddress.getByName("192.168.0.131"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5001) {
                    ...
                    if (flag) {
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender1.getHost()));

...
public class Server8 {
    ...
    static {
        try {
            ...
            privateKey = RSA.getPrivateKey("privatekey8.dat");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6008, 100, InetAddress.getByName("192.168.0.138"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5008) {
                    ...
                    if (flag) {
                        ...
                        Sender sender8 = new Sender();
                        sender8.setHost("192.168.0.138");
                        ds1.setSender(sender8);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender8.getHost()));

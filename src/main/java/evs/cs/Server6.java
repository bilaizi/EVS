...
public class Server6 {
    ...
    static {
        try {
            ...
            privateKey = RSA.getPrivateKey("privatekey6.dat");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6006, 100, InetAddress.getByName("192.168.0.136"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5006) {
                    ...
                    if (flag) {
                        ...
                        Sender sender6 = new Sender();
                        sender6.setHost("192.168.0.136");
                        ds1.setSender(sender6);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender6.getHost()));

...
public class Server2 {
    ...
    static {
        try {
            ...
            privateKey = RSA.getPrivateKey("privatekey2.dat");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6002, 100, InetAddress.getByName("192.168.0.132"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5002) {
                    ...
                    if (flag) {
                        ...
                        sender2.setHost("192.168.0.132");
                        ds1.setSender(sender2);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender2.getHost()));

...
public class Server4 {
    ...
    static {
        try {
            ...
            privateKey = RSA.getPrivateKey("privatekey4.dat");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6004, 100, InetAddress.getByName("192.168.0.134"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5004) {
                    ...
                    if (flag) {
                        ...
                        sender4.setHost("192.168.0.134");
                        ds1.setSender(sender4);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender4.getHost()));

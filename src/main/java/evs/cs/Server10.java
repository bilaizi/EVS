...
public class Server10 {
    ...
    private void init() {
        try {
            ServerSocket serverSocket = new ServerSocket(6010, 100, InetAddress.getByName("192.168.0.140"));
            while (true) {
    ...
    private class HandlerThread implements Runnable {
        ...
        public void run() {
            try {
                if (((InetSocketAddress) socket.getRemoteSocketAddress()).getPort() == 5010) {
                    ...
                    if (flag) {
                        ...
                        Sender sender10 = new Sender();
                        sender10.setHost("192.168.0.140");
                        ds1.setSender(sender10);
                        ...
                        if (Math.random() > 0.5) {
                            ...
                            hostInfoTables.removeIf(s -> Objects.equals(s.getHost(), sender10.getHost()));
